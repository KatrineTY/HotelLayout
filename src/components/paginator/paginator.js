...
$('.paginator__pagination').pagination({
  ...
  afterPaging() {
    ...
    $('.js-card__inner').on('click', function () {
      ...
      sessionStorage.setItem('number', $('.js-card__room-number', this)[0].innerText);
      if ($('.js-card__room-kind', this).length > 0) {
        sessionStorage.setItem('kind', $('.js-card__room-kind', this)[0].innerText);
      }
      sessionStorage.setItem('price', $('.js-card__room-price', this)[0].innerText.replace('₽ в сутки', ''));
      sessionStorage.setItem('guests', JSON.stringify($('.dropdown[data-type="guests"]').find('.js-input-number__input').map((i,item) => item.value)));
      
      location.href = "room-details-page.html";
    })
